...
static int INM(uint16_t a, uint16_t *r) {
    ...
    if (M == 0) {
        ...
        *r = sign < 0 ? X : 0xFDF3 - X;
        ...
        if (MOD3((uint32_t) (*r) * a) == 1) { return 0; }
        return -1;
    }
...
static int MLT(uint32_t *R, uint32_t X, uint32_t k) {
    ...
    if (DUB(&xX[2], X) != 0) { return -1; }
    if (TIA(&xX[3], X) != 0) { return -1; }
